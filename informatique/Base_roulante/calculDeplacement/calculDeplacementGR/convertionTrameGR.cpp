...
#include "defineDeplacementGR.h"

using namespace std ;

/*
 * @brief convertir une trame déplacement en une trame moteur
...
Trame_Moteur_t* convertir(Trame_BR_dpt* trameDpt, Trame_Moteur_t* trameMoteur){
    ...
    for (int i=0; i<3; i++){
        if(ratio[i]!=0){
            nbPasRoue[i] = (nbPas * div[i]/ratio[i]) - (0.1 * (nbPas * div[i]/ratio[i]));
        }
        else{
            nbPasRoue[i] = 0;
        }
    ...
    if(nbPasRoue[0]!=0){
        trameMoteur->fields.nb_steps = (uint32_t)(round(nbPasRoue[0]));
    }
    else if(nbPasRoue[1]!=0){
        trameMoteur->fields.nb_steps = (uint32_t)(round(nbPasRoue[1]));
    }
    else if(nbPasRoue[2]!=0){
        trameMoteur->fields.nb_steps = (uint32_t)(round(nbPasRoue[2]));
    }
    else{
...
//Affiche des différentes variables
/*
...
/*
 * @brief calcul un arc de cercle en fonction d'un angle
